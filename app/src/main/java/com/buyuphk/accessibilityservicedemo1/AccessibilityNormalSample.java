...
//                simulationClickByText();
    ...
    private void simulationClickById() {
        ...
        AccessibilityLog.printLog(result ? "复选框模拟点击成功" : "复选框模拟点击失败");
        if (result) {
            Log.d("debug", "复选框模拟点击成功");
        } else {
            Log.d("debug", "复选框模拟点击失败");
        }
        mHandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                ...
                AccessibilityLog.printLog(result ? "单选按钮模拟点击成功" : "单选按钮模拟点击失败");
                if (result) {
                    Log.d("debug", "单选按钮模拟点击成功");
                } else {
                    Log.d("debug", "单选按钮模拟点击失败");
                }
            }
        }, 2000);
        mHandler.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                AccessibilityLog.printLog(result ? "OnOff开关模拟点击成功" : "OnOff开关模拟点击失败");
                if (result) {
                    Log.d("debug", "OnOff开关模拟点击成功");
                } else {
                    Log.d("debug", "OnOff开关模拟点击失败");
                }
            }
        }, 4000);
        mHandler.postDelayed(new Runnable() {
...
//                AccessibilityLog.printLog(result ? "退出本页面模拟点击成功" : "退出本页面模拟点击失败");
                ...
                if (result) {
                    Log.d("debug", "返回键模拟点击成功");
                } else {
                    Log.d("debug", "返回键模拟点击失败");
                }
            }
        }, 6000);
    }
